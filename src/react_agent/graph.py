...
from react_agent.tools import get_joke, get_weather, get_news
from react_agent.utils import split_model_and_provider

# Define the function that calls the model

TOOLS = [get_weather, get_news, get_joke]
...
async def call_model(state: State, config: RunnableConfig) -> Dict[str, List[AIMessage]]:
    ...
    model_config = await split_model_and_provider(configurable.model)
    model = init_chat_model(**model_config).bind_tools(TOOLS)
...
async def human_review_node(state: State) -> Command[Literal["call_model", "tools"]]:
...
async def subgraph_node(state:State):
    return Command(
        goto="__end__",
        update={
            "messages": [
